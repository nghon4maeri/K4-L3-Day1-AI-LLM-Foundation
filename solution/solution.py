...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = (
        f"{'Prompt':<42} | {'GPT-4o Response':<42} | {'Mini Response':<42} "
        f"| {'GPT-4o Latency':<14} | {'Mini Latency':<14}"
    )
    sep = "-" * len(header)
    lines = [header, sep]
    for r in results:
        lines.append(
            f"{clip(r['prompt']):<42} | {clip(r['gpt4o_response']):<42} "
            f"| {clip(r['mini_response']):<42} | {r['gpt4o_latency']:<14.3f} "
            f"| {r['mini_latency']:<14.3f}"
        )
    return "\n".join(lines)
